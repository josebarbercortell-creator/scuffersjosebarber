...
.hero-logo {{ height: 58px; color: {BRAND['text']}; display:flex; align-items:center; min-width: 0; }}
.hero-logo svg, .hero-logo img {{ height: 58px; width: auto; max-width: 260px; object-fit: contain; display:block; }}
...
.rank-row {{ display:grid; grid-template-columns: 56px 210px 1fr 130px 140px 90px 100px; gap: 1rem; align-items: center; padding: 0.95rem 1.2rem; border-bottom: 1px solid {BRAND['border']}; transition: background 0.12s ease; }}
...
.api-banner.ok       {{ border-left: 4px solid {BRAND['neon']}; }}
.api-banner.partial  {{ border-left: 4px solid {BRAND['amber']}; }}
.api-banner.error    {{ border-left: 4px solid {BRAND['critical']}; }}
.api-banner.disabled {{ border-left: 4px solid {BRAND['border_soft']}; opacity: 0.85; }}
.api-banner .head {{ display:flex; align-items:center; gap: 0.6rem; }}
.api-banner .dot {{ width: 9px; height: 9px; border-radius: 50%; }}
.api-banner.ok .dot       {{ background: {BRAND['neon']};   box-shadow: 0 0 8px {BRAND['neon']}; }}
.api-banner.partial .dot  {{ background: {BRAND['amber']};  box-shadow: 0 0 8px {BRAND['amber']}; }}
.api-banner.error .dot    {{ background: {BRAND['critical']}; box-shadow: 0 0 8px {BRAND['critical']}; }}
.api-banner.disabled .dot {{ background: {BRAND['text_dim']}; }}
.api-banner .api-title {{ font-weight: 600; color: {BRAND['text']}; font-size: 0.96rem; }}
.api-banner .api-status {{ font-size: 0.74rem; text-transform: uppercase; letter-spacing: 0.14em; font-weight: 700; }}
.api-banner .api-meta {{ color: {BRAND['text_muted']}; font-size: 0.84rem; }}
.api-banner .api-meta b {{ color: {BRAND['text']}; }}

.shipping-grid {{ display:grid; grid-template-columns: repeat(5, 1fr); gap: 0.7rem; }}
@media (max-width: 1100px) {{ .shipping-grid {{ grid-template-columns: repeat(2, 1fr); }} }}
.shipping-stat {{
    background: {BRAND['surface']}; border: 1px solid {BRAND['border']};
    border-radius: 12px; padding: 0.85rem 1rem; text-align:center;
}}
.shipping-stat .num {{ font-family: 'Space Grotesk', sans-serif; font-size: 1.7rem; font-weight: 700; color: var(--accent, {BRAND['text']}); margin: 0.2rem 0; }}
.shipping-stat .lbl {{ color: {BRAND['text_muted']}; font-size: 0.74rem; text-transform: uppercase; letter-spacing: 0.10em; font-weight: 600; }}
.shipping-stat.ship-in_transit  {{ --accent: {BRAND['primary']}; }}
.shipping-stat.ship-delayed     {{ --accent: {BRAND['amber']}; }}
.shipping-stat.ship-exception   {{ --accent: {BRAND['critical']}; }}
.shipping-stat.ship-delivered   {{ --accent: {BRAND['neon']}; }}
.shipping-stat.ship-review      {{ --accent: {BRAND['violet']}; }}

.ship-badge {{ display:inline-flex; align-items:center; gap:0.3rem; padding: 0.18rem 0.5rem; border-radius: 5px; font-size: 0.7rem; font-weight: 700; border: 1px solid; text-transform: uppercase; letter-spacing: 0.06em; }}
.ship-badge.in_transit {{ color: #7DD3FC; background: rgba(56,189,248,0.10); border-color: rgba(56,189,248,0.35); }}
.ship-badge.delayed    {{ color: #FCD34D; background: rgba(251,191,36,0.10); border-color: rgba(251,191,36,0.35); }}
.ship-badge.exception  {{ color: #FCA5A5; background: rgba(244,63,94,0.10); border-color: rgba(244,63,94,0.35); }}
.ship-badge.lost       {{ color: #FCA5A5; background: rgba(244,63,94,0.14); border-color: rgba(244,63,94,0.45); }}
.ship-badge.delivered  {{ color: #86EFAC; background: rgba(74,222,128,0.10); border-color: rgba(74,222,128,0.35); }}
.ship-badge.review     {{ color: #D8B4FE; background: rgba(168,85,247,0.10); border-color: rgba(168,85,247,0.35); }}

/* ---------- DECISION DELTA ---------- */
.delta-grid {{ display:grid; grid-template-columns: 1fr 1fr; gap: 1rem; }}
@media (max-width: 1100px) {{ .delta-grid {{ grid-template-columns: 1fr; }} }}
.delta-card {{
    background: {BRAND['surface']}; border: 1px solid {BRAND['border']};
    border-left: 4px solid var(--delta-accent, {BRAND['primary']});
    border-radius: 12px; padding: 0.95rem 1.1rem;
}}
.delta-card.up      {{ --delta-accent: {BRAND['neon']}; }}
.delta-card.down    {{ --delta-accent: {BRAND['amber']}; }}
.delta-card.entered {{ --delta-accent: {BRAND['primary']}; }}
.delta-card.dropped {{ --delta-accent: {BRAND['text_dim']}; }}
.delta-head {{ display:flex; align-items:center; justify-content:space-between; gap: 0.6rem; margin-bottom: 0.4rem; }}
.delta-rank {{
    display:inline-flex; align-items:center; gap: 0.4rem;
    font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 1rem; color: {BRAND['text']};
}}
.delta-rank .from {{ color: {BRAND['text_dim']}; text-decoration: line-through; }}
.delta-rank .arrow {{ color: var(--delta-accent); font-size: 1.2rem; }}
.delta-rank .to    {{ color: var(--delta-accent); }}
.delta-title {{ color: {BRAND['text']}; font-weight: 600; font-size: 0.94rem; margin-top: 0.2rem; }}
.delta-because {{ color: {BRAND['text_muted']}; font-size: 0.85rem; margin-top: 0.4rem; line-height: 1.45; }}
.delta-because b {{ color: var(--delta-accent); }}

/* ---------- LOGISTICS CONTROL TOWER ---------- */
.logistics-shell {{
    margin: 2.6rem 0 2rem 0; padding: 1.4rem;
    background:
      linear-gradient(180deg, rgba(18,24,41,0.96) 0%, rgba(10,14,26,0.96) 100%);
    border: 1px solid {BRAND['border_soft']};
    border-radius: 18px;
}}
.logistics-shell .section-head {{ margin-top: 0; }}
.risk-table {{
...
.risk-table th {{
    ...
    color: {BRAND['text_muted']}; font-size: 0.72rem;
    text-transform: uppercase; letter-spacing: 0.11em;
    background: rgba(56,189,248,0.04); border-bottom: 1px solid {BRAND['border']};
}}
.risk-table td {{
    padding: 0.82rem 0.9rem; border-bottom: 1px solid {BRAND['border']};
    color: {BRAND['text']}; font-size: 0.9rem;
}}
.risk-table tr:last-child td {{ border-bottom: none; }}
.risk-table code {{ font-family: 'JetBrains Mono', monospace; color: {BRAND['text_muted']}; }}
.risk-table .num {{ text-align: right; font-family: 'JetBrains Mono', monospace; }}
.risk-pill {{
    display:inline-flex; align-items:center; justify-content:center;
    min-width: 72px; padding: 0.24rem 0.55rem; border-radius: 7px;
    font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em;
    border: 1px solid;
}}
.risk-pill.high {{ color: #FCA5A5; background: rgba(244,63,94,0.12); border-color: rgba(244,63,94,0.36); }}
.risk-pill.medium {{ color: #FCD34D; background: rgba(251,191,36,0.12); border-color: rgba(251,191,36,0.34); }}
.risk-pill.low {{ color: #86EFAC; background: rgba(74,222,128,0.10); border-color: rgba(74,222,128,0.30); }}
.risk-row-high td:first-child {{ border-left: 4px solid {BRAND['critical']}; }}
.risk-row-medium td:first-child {{ border-left: 4px solid {BRAND['amber']}; }}
.risk-row-low td:first-child {{ border-left: 4px solid {BRAND['neon']}; }}
.action-suggestion-grid {{ display:grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-top: 1rem; }}
@media (max-width: 1100px) {{ .action-suggestion-grid {{ grid-template-columns: 1fr; }} }}
.action-suggestion {{
    background: {BRAND['surface']}; border: 1px solid {BRAND['border']};
    border-left: 4px solid var(--accent, {BRAND['primary']});
    border-radius: 12px; padding: 1rem 1.1rem;
}}
.action-suggestion.high {{ --accent: {BRAND['critical']}; }}
.action-suggestion.medium {{ --accent: {BRAND['amber']}; }}
.action-suggestion.low {{ --accent: {BRAND['neon']}; }}
.action-suggestion .sku {{ font-family: 'JetBrains Mono', monospace; color: {BRAND['text_muted']}; font-size: 0.78rem; }}
.action-suggestion .title {{ color: {BRAND['text']}; font-weight: 700; font-size: 0.98rem; margin: 0.35rem 0; }}
.action-suggestion .meta {{ color: {BRAND['text_muted']}; font-size: 0.84rem; line-height: 1.45; }}
...
.sb-brand .logo {{ height: 38px; color: {BRAND['text']}; display:flex; align-items:center; min-width:0; }}
.sb-brand .logo svg, .sb-brand .logo img {{ height: 38px; width: auto; max-width: 190px; object-fit: contain; display:block; }}
...
.scuffers-footer .footer-logo {{ height: 20px; color: {BRAND['text_dim']}; display:inline-flex; align-items:center; opacity: 0.7; }}
.scuffers-footer .footer-logo svg, .scuffers-footer .footer-logo img {{ height: 20px; width: auto; max-width: 110px; object-fit: contain; display:block; }}
</style>
"""
...
def _find_logo_file() -> Optional[str]:
    names = [
        "scuffers_logo_light", "scuffers-logo-light",
        "scuffers_logo", "scuffers-logo",
        "logo_scuffers", "logo-scuffers",
        "logo", "scuffers",
    ]
    exts = [".svg", ".png", ".webp", ".jpg", ".jpeg"]
    folders = [ASSETS_DIR, PROJECT_DIR]
...
def load_logo(variant: str = "light") -> str:
    """Returns inline HTML for the Scuffers logo. Drop the real logo as:
       assets/scuffers_logo.svg/png/webp/jpg or directly in the project root.
       Falls back to a minimal embedded wordmark.
    """
    ...
    if logo_path and logo_path.lower().endswith(".svg"):
        with open(logo_path, "r", encoding="utf-8") as f:
            svg = f.read()
        if "<svg" in svg:
            svg = svg.replace("<svg", '<svg class="scuffers-logo-svg"', 1)
        return f'<span style="color:{color}; display:inline-block; line-height:0;">{svg}</span>'
    if logo_path:
        with open(logo_path, "rb") as f:
            b64 = base64.b64encode(f.read()).decode()
        return f'<img src="data:{_image_mime(logo_path)};base64,{b64}" alt="Scuffers" />'
...
SHIPPING_STATUS_ENDPOINT = "https://lkuutmnykcnbfmbpopcu.functions.supabase.co/api/shipping-status/{order_id}"
SHIPPING_CANDIDATE_ID = "SCF-2026-XXXX"
SHIPPING_RISK_STATUSES = {"delayed", "exception", "lost"}
SHIPPING_ORDER_ACTIONS = {"EXPEDITE_ORDER", "PROACTIVE_CONTACT", "REVIEW_ORDER"}
...
class Action:
    ...
    # Shipping API enrichment (set later by apply_shipping_signal)
    shipping_status: Optional[str] = None     # in_transit / delayed / exception / lost / delivered
    shipping_delta: float = 0.0               # score change from shipping API
    shipping_tags: List[str] = field(default_factory=list)
    shipping_review: bool = False             # requires_manual_review from API
    score_pre_api: Optional[float] = None     # snapshot before shipping boost
...
def rank_actions(all_actions, top_n=10):
    cols = ["rank", "action_type", "target_id", "title", "tags", "impact_bullets",
            ...
            "shipping_status", "delay_risk", "delay_reason", "requires_manual_review",
            "shipping_priority_boost", "final_score", "before_score",
            "shipping_delta", "shipping_tags", "shipping_review", "score_pre_api", "shipping_api_ok"]
    if not all_actions:
    ...
    for a in sorted(all_actions, key=lambda x: x.score, reverse=True):
        ...
        rows.append({
            ...
            "shipping_status": a.shipping_status or "not_checked",
            "delay_risk": 0.0,
            "delay_reason": "",
            "requires_manual_review": bool(a.shipping_review),
            "shipping_priority_boost": 0.0,
            "final_score": round(a.score, 1),
            "before_score": round(a.score, 1),
            "shipping_delta": round(a.shipping_delta, 1),
            "shipping_tags": list(a.shipping_tags),
            "shipping_review": a.shipping_review,
            "score_pre_api": (round(a.score_pre_api, 1) if a.score_pre_api is not None else None),
            "shipping_api_ok": False,
        })
        if len(rows) >= top_n:
            break
...
def render_shipping_badge(status: str, review: bool = False) -> str:
    clean_status = str(status or "not_checked").lower().strip()
    if clean_status in {"", "none", "nan"}:
        clean_status = "unknown"
    if clean_status in {"not_checked", "unknown"}:
        return '<span class="ship-badge review">NO SIGNAL</span>' if review else '<span class="ship-badge review">N/A</span>'
    css_status = clean_status if clean_status in {"in_transit", "delayed", "exception", "lost", "delivered"} else "review"
    label = clean_status.replace("_", " ")
    review_dot = " · REVIEW" if review else ""
    return f'<span class="ship-badge {css_status}">{escape(label)}{review_dot}</span>'
...
def render_top_table(df: pd.DataFrame) -> str:
    ...
    head = """<div class="rank-row head"><div>#</div><div>Acción</div><div>Caso</div><div>Equipo · Confianza</div><div>Shipping</div><div>Auto</div><div style="text-align:right;">Score</div></div>"""
    rows_html = [head]
    for _, r in df.iterrows():
        ...
        shipping_badge = render_shipping_badge(r.get("shipping_status", "not_checked"), r.get("requires_manual_review", False))
        boost = float(r.get("shipping_priority_boost", 0) or 0)
        boost_html = f'<div class="conf-text" style="margin-top:0.28rem;">shipping {boost:+.0f}</div>' if boost else ""
        ...
        <div class="rank-row">
          ...
          <div>{shipping_badge}{boost_html}</div>
          <div>{auto_cell}</div>
...
def render_shipping_decision_delta(df: pd.DataFrame) -> str:
    if df.empty or "shipping_priority_boost" not in df.columns:
        return ""
    changed = df[pd.to_numeric(df["shipping_priority_boost"], errors="coerce").fillna(0) != 0].copy()
    if changed.empty:
        return _fmt(f"""
        <div class="api-banner disabled">
          <div class="head">
            <span class="dot"></span>
            <div>
              <div class="api-title">Decision updated after shipping signal</div>
              <div class="api-meta">No score changes from shipping data for the current ranking.</div>
            </div>
          </div>
        </div>""")
    ...
    for _, row in changed.head(6).iterrows():
        before = float(row.get("before_score", row.get("score", 0)) or 0)
        after = float(row.get("final_score", row.get("score", 0)) or 0)
        boost = float(row.get("shipping_priority_boost", 0) or 0)
        tone = "up" if boost > 0 else "down"
        reason = row.get("delay_reason") or row.get("shipping_status") or "shipping signal"
        cards.append(_fmt(f"""
        <div class="delta-card {tone}">
          <div class="delta-head">
            <div class="delta-rank">
              <span class="from">{before:.0f}</span>
              <span class="arrow">→</span>
              <span class="to">{after:.0f}</span>
            </div>
            {render_shipping_badge(row.get("shipping_status", "unknown"), row.get("requires_manual_review", False))}
          </div>
          <div class="delta-title">{escape(str(row.get("title", row.get("target_id", ""))))}</div>
          <div class="delta-because">
            before_score <b>{before:.1f}</b> · after_score <b>{after:.1f}</b> · boost <b>{boost:+.0f}</b><br/>
            {escape(str(reason))}
          </div>
        </div>"""))
    return f'<div class="delta-grid">{"".join(cards)}</div>'


def build_logistics_frame(inventory_df: pd.DataFrame, master_df: pd.DataFrame,
                          campaigns_df: pd.DataFrame, ranked_df: pd.DataFrame) -> pd.DataFrame:
    if inventory_df is None or inventory_df.empty or "sku" not in inventory_df.columns:
        return pd.DataFrame()

    cols = ["sku", "inventory_available_units", "inventory_reserved_units"]
    optional = [c for c in ["product_name", "category"] if c in inventory_df.columns]
    logistics = inventory_df[cols + optional].copy()
    logistics["available"] = pd.to_numeric(logistics["inventory_available_units"], errors="coerce").fillna(0)
    logistics["reserved"] = pd.to_numeric(logistics["inventory_reserved_units"], errors="coerce").fillna(0)
    logistics["gap"] = (logistics["reserved"] - logistics["available"]).clip(lower=0)
    logistics["overbook_ratio"] = np.where(
        logistics["available"] > 0,
        logistics["reserved"] / logistics["available"],
        np.where(logistics["reserved"] > 0, np.inf, 0),
    )
    logistics["active_campaign"] = False
    if campaigns_df is not None and not campaigns_df.empty and "target_sku" in campaigns_df.columns:
        active_campaigns = campaigns_df
        if "status" in active_campaigns.columns:
            active_campaigns = active_campaigns[active_campaigns["status"].astype(str).str.lower() == "active"]
        logistics["active_campaign"] = logistics["sku"].astype(str).isin(active_campaigns["target_sku"].dropna().astype(str))

    logistics["delayed_orders"] = 0
    logistics["max_delay_risk"] = 0.0
    if (
        master_df is not None and not master_df.empty and {"order_id", "sku"}.issubset(master_df.columns)
        and ranked_df is not None and not ranked_df.empty and {"target_id", "shipping_status", "delay_risk"}.issubset(ranked_df.columns)
    ):
        shipping_by_order = ranked_df[["target_id", "shipping_status", "delay_risk"]].copy()
        shipping_by_order["order_id"] = shipping_by_order["target_id"].astype(str)
        shipping_by_order["delay_risk"] = pd.to_numeric(shipping_by_order["delay_risk"], errors="coerce").fillna(0)
        shipping_by_order = shipping_by_order[
            shipping_by_order["shipping_status"].astype(str).str.lower().isin(SHIPPING_RISK_STATUSES)
            | (shipping_by_order["delay_risk"] > 0.6)
        ]
        if not shipping_by_order.empty:
            order_skus = master_df[["order_id", "sku"]].copy()
            order_skus["order_id"] = order_skus["order_id"].astype(str)
            impacted = order_skus.merge(shipping_by_order[["order_id", "delay_risk"]], on="order_id", how="inner")
            shipping_signal = impacted.groupby("sku").agg(
                delayed_orders=("order_id", "nunique"),
                max_delay_risk=("delay_risk", "max"),
            ).reset_index()
            logistics = logistics.merge(shipping_signal, on="sku", how="left", suffixes=("", "_ship"))
            logistics["delayed_orders"] = logistics["delayed_orders_ship"].fillna(logistics["delayed_orders"]).astype(int)
            logistics["max_delay_risk"] = logistics["max_delay_risk_ship"].fillna(logistics["max_delay_risk"])
            logistics = logistics.drop(columns=[c for c in ["delayed_orders_ship", "max_delay_risk_ship"] if c in logistics.columns])

    def risk_level(row) -> str:
        if row["gap"] > 0 or row["available"] == 0 or row["max_delay_risk"] > 0.6:
            return "High"
        if row["overbook_ratio"] >= 0.75 or row["delayed_orders"] > 0:
            return "Medium"
        return "Low"

    logistics["risk_level"] = logistics.apply(risk_level, axis=1)
    level_order = {"High": 3, "Medium": 2, "Low": 1}
    logistics["risk_sort"] = logistics["risk_level"].map(level_order).fillna(0)
    return logistics.sort_values(["gap", "risk_sort", "reserved"], ascending=False).reset_index(drop=True)


def render_critical_skus_table(logistics_df: pd.DataFrame, limit: int = 12) -> str:
    if logistics_df.empty:
        return _fmt(f"""
        <div class="rank-table" style="padding:1.3rem; color:{BRAND['text_muted']};">
          No inventory data available for logistics analysis.
        </div>""")

    rows = []
    for _, r in logistics_df.head(limit).iterrows():
        risk = str(r.get("risk_level", "Low"))
        risk_cls = risk.lower()
        delayed_note = ""
        if int(r.get("delayed_orders", 0) or 0) > 0:
            delayed_note = f'<span class="ship-badge delayed" style="margin-left:0.45rem;">{int(r["delayed_orders"])} delayed</span>'
        rows.append(_fmt(f"""
        <tr class="risk-row-{risk_cls}">
          <td><code>{escape(str(r["sku"]))}</code>{delayed_note}</td>
          <td class="num">{int(r["available"])}</td>
          <td class="num">{int(r["reserved"])}</td>
          <td class="num">{int(r["gap"])}</td>
          <td><span class="risk-pill {risk_cls}">{escape(risk)}</span></td>
        </tr>"""))
...
def render_logistics_suggestions(logistics_df: pd.DataFrame, limit: int = 6) -> str:
    risky = logistics_df[(logistics_df["gap"] > 0) | (logistics_df["risk_level"].isin(["High", "Medium"]))].head(limit)
    if risky.empty:
        return _fmt(f"""
        <div class="api-banner ok">
          <div class="head"><span class="dot"></span><div>
            <div class="api-title">Action suggestions</div>
            <div class="api-meta">No immediate replenishment or fulfillment action required.</div>
          </div></div>
        </div>""")
    ...
    for _, r in risky.iterrows():
        actions = []
        if r["gap"] > 0 or r["available"] == 0:
            actions.append("Replenish stock")
        if bool(r.get("active_campaign", False)):
            actions.append("Pause campaign")
        if int(r.get("delayed_orders", 0) or 0) > 0 or float(r.get("max_delay_risk", 0) or 0) > 0.6:
            actions.append("Prioritize fulfillment")
        if not actions:
            actions.append("Prioritize fulfillment")
        risk_cls = str(r.get("risk_level", "Low")).lower()
        cards.append(_fmt(f"""
        <div class="action-suggestion {risk_cls}">
          <div class="sku">{escape(str(r["sku"]))}</div>
          <div class="title">{escape(" · ".join(actions))}</div>
          <div class="meta">
            Gap {int(r["gap"])} units · reserved {int(r["reserved"])} vs available {int(r["available"])}
            {f' · {int(r["delayed_orders"])} delayed order(s)' if int(r.get("delayed_orders", 0) or 0) else ''}
          </div>
        </div>"""))
    return f'<div class="action-suggestion-grid">{"".join(cards)}</div>'
...
# ============================================================================
# 7) SHIPPING STATUS API
# ============================================================================
def _empty_shipping_status(error: Optional[str] = None) -> Dict[str, Any]:
    return {
        "shipping_status": "unknown",
        "delay_risk": 0.0,
        "delay_reason": error or "",
...
def fetch_shipping_status(order_id) -> Dict[str, Any]:
    """Fetch shipping signal for one order. Failures are neutral for scoring."""
    clean_order_id = str(order_id or "").strip()
    if not clean_order_id or clean_order_id.lower() in {"nan", "none", "?"}:
        return _empty_shipping_status("missing order_id")

    try:
        resp = requests.get(
            SHIPPING_STATUS_ENDPOINT.format(order_id=clean_order_id),
            headers={"X-Candidate-Id": SHIPPING_CANDIDATE_ID},
            timeout=3,
        )
        resp.raise_for_status()
        payload = resp.json() if resp.content else {}
    except requests.RequestException as exc:
        return _empty_shipping_status(f"api unavailable: {exc.__class__.__name__}")
    except ValueError:
        return _empty_shipping_status("invalid api response")

    status = str(payload.get("shipping_status") or payload.get("status") or "unknown").strip().lower()
    try:
        delay_risk = float(payload.get("delay_risk", 0) or 0)
    except (TypeError, ValueError):
        delay_risk = 0.0
    delay_risk = max(0.0, min(delay_risk, 1.0))

    return {
        "shipping_status": status or "unknown",
        "delay_risk": delay_risk,
        "delay_reason": str(payload.get("delay_reason") or payload.get("reason") or ""),
        "requires_manual_review": _to_bool(payload.get("requires_manual_review", False)),
        "shipping_api_ok": True,
    }


def shipping_priority_boost(row: pd.Series) -> float:
    boost = 0.0
    status = str(row.get("shipping_status", "unknown")).lower()
    if float(row.get("delay_risk", 0) or 0) > 0.6:
        boost += 10
    if status in SHIPPING_RISK_STATUSES:
        boost += 20
    if bool(row.get("requires_manual_review", False)):
        boost += 15
    if status == "delivered":
        boost -= 10
    return boost
...
def enrich_top_ranked_orders_with_shipping(ranked_df: pd.DataFrame, top_k: int = 10) -> pd.DataFrame:
    if ranked_df is None or ranked_df.empty:
        return ranked_df

    df = ranked_df.copy()
    for col, default in {
        "shipping_status": "not_checked",
        "delay_risk": 0.0,
        "delay_reason": "",
        "requires_manual_review": False,
        "shipping_priority_boost": 0.0,
    ...
    df["before_score"] = pd.to_numeric(df["score"], errors="coerce").fillna(0.0)
    df["final_score"] = df["before_score"]
    top_df = df.head(top_k)
    top_order_mask = top_df["action_type"].isin(SHIPPING_ORDER_ACTIONS) & top_df["target_id"].notna()
    top_order_indices = top_df.index[top_order_mask]

    for idx in top_order_indices:
        signal = fetch_shipping_status(df.at[idx, "target_id"])
        for key in ["shipping_status", "delay_risk", "delay_reason", "requires_manual_review", "shipping_api_ok"]:
            df.at[idx, key] = signal[key]

    df["shipping_priority_boost"] = df.apply(shipping_priority_boost, axis=1)
    df["final_score"] = df["before_score"] + df["shipping_priority_boost"]
    df["score"] = df["final_score"]
    df = df.sort_values(["final_score", "before_score"], ascending=False).reset_index(drop=True)
...
ranked = enrich_top_ranked_orders_with_shipping(ranked, top_k=10)
...
if not ranked.empty:
    api_checked = int((ranked.get("shipping_status", pd.Series(dtype=str)).astype(str) != "not_checked").sum())
    api_ok = int(ranked.get("shipping_api_ok", pd.Series(dtype=bool)).fillna(False).sum())
    banner_cls = "ok" if api_checked and api_ok == api_checked else ("partial" if api_checked else "disabled")
    st.markdown(_fmt(f"""
    <div class="section-head">
      <div>
        <div class="eyebrow">Shipping signal</div>
        <h2>Decision updated after shipping signal</h2>
      </div>
      <div class="meta">before_score vs after_score<br/>API calls limited to top 10 orders</div>
    </div>
    <div class="api-banner {banner_cls}">
      <div class="head">
        <span class="dot"></span>
        <div>
          <div class="api-title">Shipping Status API</div>
          <div class="api-meta"><b>{api_ok}/{api_checked}</b> shipping signals applied · failures stay neutral</div>
        </div>
      </div>
    </div>"""), unsafe_allow_html=True)
    st.markdown(render_shipping_decision_delta(ranked), unsafe_allow_html=True)
...
# Row 2 — Logistics Control Tower
logistics = build_logistics_frame(inventory, master, campaigns, ranked)
if not logistics.empty:
    skus_at_risk = int((logistics["gap"] > 0).sum())
    critical_skus = int((logistics["available"] == 0).sum())
    total_available = float(logistics["available"].sum())
    total_reserved = float(logistics["reserved"].sum())
    overbook_ratio = (total_reserved / total_available * 100) if total_available > 0 else 0
    total_units_at_risk = int(logistics["gap"].sum())
    delayed_skus = int((logistics["delayed_orders"] > 0).sum())

    logistics_kpis = (
        render_kpi_card("📦", "SKUs at risk", skus_at_risk,
                        "reserved units exceed available stock", "bad" if skus_at_risk else "primary")
        + render_kpi_card("⛔", "Critical SKUs", critical_skus,
                          "available stock is zero", "bad" if critical_skus else "primary")
        + render_kpi_card("⚖", "Overbook ratio", f"{overbook_ratio:.0f}%",
                          "reserved / available across inventory", "warn" if overbook_ratio >= 85 else "primary")
        + render_kpi_card("🚚", "Total units at risk", total_units_at_risk,
                          f"{delayed_skus} SKUs linked to delayed shipping signals", "warn" if total_units_at_risk else "primary")
    )
    ...
    <div class="logistics-shell">
      <div class="section-head">
        <div>
          ...
          <h2>Logistics Control Tower</h2>
        </div>
        <div class="meta">inventory pressure · fulfillment risk<br/>shipping-aware prioritization</div>
      </div>
      <div class="kpi-grid">{logistics_kpis}</div>
    </div>"""), unsafe_allow_html=True)

    st.markdown(_fmt("""
    <div class="section-head">
      <div>
        <div class="eyebrow">Critical SKUs</div>
        <h2>Inventory gaps ranked by operational risk</h2>
      </div>
      <div class="meta">sorted by gap descending<br/>shipping delays raise risk level</div>
    </div>"""), unsafe_allow_html=True)
    st.markdown(render_critical_skus_table(logistics), unsafe_allow_html=True)

    gap_chart = logistics.sort_values(["gap", "risk_sort", "reserved"], ascending=False).head(10).copy()
    gap_chart["risk_label"] = gap_chart["risk_level"].astype(str)
    gap_chart["delayed_label"] = np.where(
        gap_chart["delayed_orders"] > 0,
        " · delayed orders: " + gap_chart["delayed_orders"].astype(int).astype(str),
        "",
    )
    fig = px.bar(
        gap_chart.sort_values("gap", ascending=True),
        x="gap",
        y="sku",
        orientation="h",
        color="risk_label",
        color_discrete_map={"High": "#F43F5E", "Medium": "#FBBF24", "Low": "#4ADE80"},
        text="gap",
        title="Top 10 SKUs by inventory gap",
        custom_data=["available", "reserved", "risk_level", "delayed_orders"],
    )
    fig.update_traces(
        marker_line_width=0,
        ...
        hovertemplate=(
            "<b>%{y}</b><br>"
            "Gap: %{x:.0f} units<br>"
            "Available: %{customdata[0]:.0f}<br>"
            "Reserved: %{customdata[1]:.0f}<br>"
            "Risk: %{customdata[2]}<br>"
            "Delayed orders: %{customdata[3]:.0f}<extra></extra>"
        ),
    )
    fig.update_layout(
        height=560,
        ...
        yaxis_title=None,
        bargap=0.28,
        margin=dict(t=70, b=55, l=90, r=45),
    )
    fig.update_xaxes(showgrid=True, gridcolor="rgba(31,42,68,0.45)", zeroline=False)
    ...
    <div class="section-head">
      <div>
        <div class="eyebrow">Action Suggestions</div>
        <h2>Recommended logistics decisions</h2>
      </div>
      <div class="meta">replenishment · campaign pressure<br/>fulfillment sequencing</div>
    </div>"""), unsafe_allow_html=True)
    st.markdown(render_logistics_suggestions(logistics), unsafe_allow_html=True)
